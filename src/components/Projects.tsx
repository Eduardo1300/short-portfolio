...
const ProjectCard: React.FC<{ 
  ...
  isFeatured?: boolean;
  altText?: string;
}> = ({ title, stack, description, repo, demo, icon, isFeatured = false, altText }) => (
...
const Projects: React.FC<{ locale: Locale }> = ({ locale }) => {
  ...
  // Icons for projects - reordered to match display order
  const projectIcons = [
  ...
  return (
    <section className="mb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8" role="list">
        {/* Featured Project - Tienda Online */}
