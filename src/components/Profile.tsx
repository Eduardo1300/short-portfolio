...
const Profile: React.FC<{ locale: Locale }> = ({ locale }) => {
  ...
  return (
    <article className="rounded-xl border border-white/10 bg-white/[0.04] p-4 shadow-sm h-full relative overflow-hidden">
      {/* Elemento decorativo */}
      ...
      <div className="relative">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h2 className="text-[clamp(12px,1.1vw,15px)] font-semibold text-slate-100">{t.profileSection.title}</h2>
        </div>
        
        <div className="space-y-3">
          <p className="text-[clamp(10px,1vw,13px)] text-slate-300 leading-relaxed">
